...
def drawFrameFeatures(frame, prevPts, currPts, frameIdx):
    ...
    for i in range(len(currPts)-1):
        cv2.circle(currFrameRGB, (int(tuple(currPts[i])[0]) , int(tuple(currPts[i])[1])), radius=3, color=(200, 100, 0))
        cv2.line(currFrameRGB, (int(tuple(prevPts[i])[0]) , int(tuple(prevPts[i])[1])), (int(tuple(currPts[i])[0]) , int(tuple(currPts[i])[1])), color=(200, 100, 0))
